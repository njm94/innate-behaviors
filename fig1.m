...
line([0, 120*fs], [N+2 N+2], 'LineWidth', 3, 'Color', [0 0 0])
axis([0 size(event_raster,2) 0 N+2])
...
% figure, bar(mean(all_relfreq, 3)')
all_spon_relfreq = reshape(all_relfreq(:,1:2,:), 4, []);
all_evoked_relfreq = reshape(all_relfreq(:,3:6,:), 4, []);


% all_spon_relfreq = squeeze(mean(all_relfreq(:, 1:2, :),2));
% all_evoked_relfreq = squeeze(mean(all_relfreq(:,3:end, :),2));
