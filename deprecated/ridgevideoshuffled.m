...
rng("default")
trial_offset = 7;
ridx = randperm(6)+trial_offset;
fileID = fopen('expt1_datalist.txt','r');
formatSpec = '%s';
data_list = textscan(fileID, formatSpec);
for j = 8:13%length(data_list{1})
    data_dir = data_list{1}{j};
    shuf_data_dir = data_list{1}{ridx(j-trial_offset)};
    disp(['Starting ' data_dir])
    fPath = [data_dir filesep 'ridge_outputs_video' filesep];
    if ~isdir(fPath), mkdir(fPath); end
    if isfile([fPath, 'shuffled_summary.fig'])
    ...
    shuf_exp_date = strfind(shuf_data_dir, filesep);
    shuf_exp_date = shuf_data_dir(shuf_exp_date(end)+1:end);
    fs = 90;

    try
        brain_file = [data_dir, filesep, getAllFiles(data_dir, 'cam0_svd')];
        beh_file = [shuf_data_dir, filesep, getAllFiles(shuf_data_dir, [shuf_exp_date, '_svd'])];
        ME_file = [data_dir, filesep, get_file_with_str(shuf_data_dir, 'MEsvd')];
...
%         dlc_speed_file = [data_dir, filesep, getAllFiles(data_dir, 'speed.csv')];
        timestamp_file = [shuf_data_dir, filesep, getAllFiles(shuf_data_dir, 'trim.txt')];
...
%     Vme = s*V(:, 1:trial_length);
    ...
    save([fPath, char(datetime('now', 'Format', 'yyyy-MM-dd-HH-mm-ss')), '_cvFull_shuffle.mat'], 'Vfull', 'fullBeta', 'fullR', 'fullIdx', 'fullRidge', 'fullLabels', '-v7.3'); %save some results
...
%         end
    ...
    save([fPath, char(datetime('now', 'Format', 'yyyy-MM-dd-HH-mm-ss')), '_cvReduced_suffle.mat'], 'Vreduced', 'reducedBeta', 'reducedR', 'reducedIdx', 'reducedRidge', 'reducedLabels', '-v7.3'); %save some results
    ...
    savefig(gcf, [fPath 'shuffled_summary.fig'])
end
  %%
