...
for j = thy1_idx%1:length(data_list{1})+1
     try
...
%         grooming_file = [data_dir, filesep, 'grooming_events_roi_filtered.mat'];
    ...
    for ii = 1:size(behavior_frames,2)
        [~, behavior_idx] = aggregate(eval(bvars(ii)));
        if ~isempty(behavior_idx)
            for jj = 1:size(behavior_idx,1)
                cell_size = size(behavior_frames{ii},2);
                behavior_frames{ii}{cell_size+1} = dFF(:,:,behavior_idx(jj,1):behavior_idx(jj,2));
            end
        end
    end
