...
% states = ["Stop", "Elliptical", "Bilateral Asymmetric", "Bilateral", "Unilateral"];

tmat = zeros(numel(states));
% state_counter = zeros(size(states));
spontaneous_sessions = [14,15,20,21];
evoked_sessions = [16:19,22:25];

for j = evoked_sessions%14:length(data_list{1})
    ...
    for ii = 1:size(idx,1)
        ...
        for jj = 1:length(tmp)
            if jj == 1
                ...
                last_event = tmp(jj);
%                 state_counter(1) = state_counter(1) + 1;
            else
                 if tmp(jj)~=tmp(jj-1) && tmp(jj) ~= 1
                     tmat(last_event, tmp(jj)) = tmat(last_event, tmp(jj)) + 1;
                     last_event = tmp(jj);
%                      state_counter(tmp(jj)) = state_counter(tmp(jj)) + 1;
                 end
            end
        end
